...
const ServiceBenevolat = () => {
  ...
  const deleteService = (serviceId) => {
    serviceBenevolatService.deleteService(serviceId)
      .then(res => {
        setServices((current) => current.filter(service => service.id !== serviceId));
        notify('Service de bénévolat supprimé avec succès');
      })
      .catch(err => {
        console.error(err);
        setError(err.message || 'Une erreur s\'est produite');
        notify('Une erreur s\'est produite lors de la suppression du service de bénévolat');
      });
  }
    ...
    <div className="shadow-lg rounded-lg overflow-hidden mx-4 md:mx-10">
      <div className="bg-white p-6 flex flex-col md:flex-row md:items-center">
        <div className="mb-4 md:mb-0 md:mr-6">
          <h2 className="text-2xl font-bold mb-4 md:mb-0">Gestion des Services de Bénévolat</h2>
          <input
            type="text"
          ...
          />
          {services.length === 0 ? (
            <p>Aucun service de bénévolat trouvé</p>
          ) : (
                ...
                <tbody className="bg-white">
                  {filteredServices.map((service) => (
                    <tr key={service.id} className="border-b border-gray-200">
                      <td className="py-2 md:py-4 px-4 md:px-6">{service.id}</td>
                      <td className="py-2 md:py-4 px-4 md:px-6">{service.name}</td>
        ...
        <div className="md:border-l md:border-gray-400 md:pl-6 md:mt-0 mt-4">
          <Link to="/admin/services/add/">
            <button className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mb-4">
              Ajouter un Service de Bénévolat
            </button>
          </Link>
          <ToastContainer />
        </div>
...
export default ServiceBenevolat;
